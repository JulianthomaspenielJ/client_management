...
class USERMODEL extends APIRESPONSE
{
    ...
    public function getUserDetails($data, $loginData)
    {
        try {
            ...
            // $sql = "SELECT id,user_name,email_id,phone,created_date, updated_date FROM tbl_users WHERE status = 1  ORDER BY id ASC LIMIT " . $start_index . "," . $end_index . "";
            $sql="SELECT u.id, u.user_name, u.email_id, u.phone,c.id AS client_id, c.client_name,u.created_by FROM tbl_users u INNER JOIN tbl_client_user_map m ON u.id = m.user_id
            INNER JOIN tbl_client c ON m.client_id = c.id WHERE u.status = 1 AND u.created_by =$loggedId  ORDER BY u.id ASC LIMIT $start_index, $end_index";
            // print_r($sql);exit;
            $result = $db->query($sql);
    ...
    public function getUser($data)
    {
        try {
            ...
            $responseArray = "";
            $db = $this->dbConnect();
            $sql = "SELECT rl.id,rl.user_name,rl.user_name,rl.email_id,rl.phone,cl.client_name FROM tbl_client_user_map AS csm JOIN tbl_users AS rl JOIN tbl_client AS cl ON csm.client_id = cl.id AND csm.user_id=rl.id WHERE csm.status=1 AND rl.id =$id ";
    ...
    {       
        ...
        try {
			...
			$sql = "SELECT id FROM tbl_users WHERE  status = 1 AND email_id = '" . $userData['emailId'] . "' OR user_name = '".$userData['user_name']."'";
			...
			$insertQuery = "INSERT INTO tbl_users (user_name, email_id,password, phone, created_by, created_date) VALUES ('" . $user_name . "','" . $userData['emailId'] . "','" . $hashed_password . "','" .$userData['phone'] . "','" . $loginData['user_id'] . "','$dateNow')";
    ...
    private function updateUser($data, $loginData)
    {
        ...
        try {
            ...
            $dateNow = date("Y-m-d H:i:s");
            // $updateQuery = "UPDATE tbl_users SET user_name = '" . $data['user_name'] . "', email_id = '" . $data['email_id'] . "',phone = '" . $data['phone'] . "',updated_by = '" . $loginData['id'] . "',updated_date = '$dateNow' WHERE id = " . $data['id'] . "";
            $updateQuery ="UPDATE tbl_users SET user_name = '" . $data['user_name'] . "',email_id = '" . $data['email_id'] . "',
        phone = '" . $data['phone'] . "',updated_by = '" . $loginData['id'] . "',updated_date = '$dateNow',status = 1
        WHERE id = " . $data['id'] . "";
    ...
    private function deleteUser($data)
    {
        ...
        try {
            ...
            $deleteQuery="UPDATE tbl_users AS u, tbl_client_user_map AS cum SET u.status = 0,cum.status = 0
    ...
    private function getTotalCount($loginData)
    {
        try {
            $db = $this->dbConnect();
            $sql = "SELECT * FROM tbl_users WHERE status = 1 and created_by = " . $loginData['id'] . "";
            $result = $db->query($sql);
            $row_cnt = mysqli_num_rows($result);
            return $row_cnt;
        } catch (Exception $e) {
