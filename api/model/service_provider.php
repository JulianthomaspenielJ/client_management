...
class SERVICEPROVIDERMODEL extends APIRESPONSE
{
    ...
    public function getServiceDetails($data, $loginData)
    {
        try {
            ...
            $sql = "SELECT id,service_name,service_url,password FROM tbl_service_provider WHERE status = 1 and created_by = " . $loginData['user_id'] . " ORDER BY id ASC LIMIT " . $start_index . "," . $end_index . "";
            $result = $db->query($sql);
